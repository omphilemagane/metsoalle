...
const init = async () => {
    ...
    for (let i = 0; i < stat["length"]; i++) {
        if (stat[i].isFile() == true) {
            const extension = stat[i]["pathname"].split("/").slice(-1)[0].split(".").slice(-1)[0];

            memo["/" + stat[i]["pathname"]] = {};

            memo["/" + stat[i]["pathname"]]["data"] = await new Promise((resolve, reject) => {
                fs.readFile(__dirname + "/" + stat[i]["pathname"], (error, content) => {
                    return error == null ? resolve(content) : reject(new Error(error["message"]));
                });
            });

            memo["/" + stat[i]["pathname"]]["mime"] = mime[extension];
        }
    }
};
...
const call = async (request, response) => {
    ...
    if (request["url"]["pathname"] == "/favicon.ico") {
        request["url"]["pathname"] = "/icons/black.png";
    ...
    if (request["url"]["pathname"] == "/robots.txt") {
        ...
        response.write('User-Agent: *\nDisallow: /\n\nSitemap: https://res.metsoalle.com/sitemap.xml');
