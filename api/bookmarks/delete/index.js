...
module.exports = async (request, response) => {
    ...
    try {
        record = await new Promise((resolve, reject) => {
            fs.readFile("/home/" + account["account-id"] + "/likes/" + request["url"]["searchParams"].get("like-id") + "/index.json", (error, content) => {
                return error = null ? resolve(JSON.parse(content)) : reject(new Error(error["message"]));
            });
        });
    } catch (error) {
    ...
    await new Promise((resolve, reject) => {
        fs.rm("/home/" + record["foreign-id"] + "/posts/" + record["post-id"] + "/likes/" + record["like-id"] + "/", { recursive: true }, (error) => {
            return error = null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.rm("/home/" + record["account-id"] + "/likes/" + record["like-id"] + "/", { recursive: true }, (error) => {
            return error = null ? resolve() : reject(new Error(error["message"]));
        });
    });
