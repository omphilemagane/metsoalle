...
const self = async (request, response, account, session, sessiex) => {
    ...
    try {
        record = await new Promise((resolve, reject) => {
            fs.readFile("/home/" + account["account-id"] + "/posts/" + request["url"]["searchParams"].get("post-id") + "/likes/" + request["url"]["searchParams"].get("like-id") + "/index.json", (error, content) => {
...
const user = async (request, response, account, session, sessiex) => {
    ...
    try {
        record = await new Promise((resolve, reject) => {
            fs.readFile("/home/" + theuser["account-id"] + "/posts/" + request["url"]["searchParams"].get("post-id") + "/likes/" + request["url"]["searchParams"].get("like-id") + "/index.json", (error, content) => {
                return error == null ? resolve(JSON.parse(content)) : reject(new Error(error["message"]));
            });
        });
    } catch (error) {
...
module.exports = async (request, response) => {
    ...
    if (request["url"]["searchParams"].has("like-id") == false || /^[0-9a-f]{128}$/.test(request["url"]["searchParams"].get("like-id")) == false) {
        response.writeHead(400, { "Content-Type": "application/json" });
