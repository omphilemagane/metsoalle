...
const self = async (request, response, account, session, sessiex) => {
    ...
    record["like-id"] = await new Promise((resolve, reject) => {
    ...
    record["like-type"] = "STANDARD";

    record["like-status"] = "ACTIVE";
    ...
    await new Promise((resolve, reject) => {
        fs.mkdir("/home/" + record["account-id"] + "/likes/" + record["like-id"] + "/", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.writeFile("/home/" + record["account-id"] + "/likes/" + record["like-id"] + "/index.json", JSON.stringify(record, null, 4), (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.mkdir("/home/" + record["foreign-id"] + "/posts/" + record["post-id"] + "/likes/" + record["like-id"] + "/", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.symlink("/home/" + record["account-id"] + "/likes/" + record["like-id"] + "/index.json", "/home/" + record["foreign-id"] + "/posts/" + record["post-id"] + "/likes/" + record["like-id"] + "/index.json", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });
