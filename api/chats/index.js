...
module.exports = async (request, response) => {
    ...
    record = await new Promise((resolve, reject) => {
        fs.readdir("/home/" + account["account-id"] + "/chats/", (error, content) => {
    ...
    record = await Promise.all(record.map((element, index) => {
        return new Promise((resolve, reject) => {
            fs.readFile("/home/" + account["account-id"] + "/chats/" + element + "/index.json", (error, content) => {
                return error = null ? resolve(JSON.parse(content)) : reject(new Error(error["message"]));
            });
        });
    }));
    ...
    record = record.map((element, index) => {
        return {
            "chat-id": element["chat-id"],
            "account-id": element["account-id"],
            "foreign-id": element["foreign-id"],
            "chat-type": element["chat-type"],
            "chat-status": element["chat-status"],
