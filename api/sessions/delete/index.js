...
module.exports = async (request, response) => {
    ...
    if (request["url"]["searchParams"].has("session-id") == false || /^[0-9a-f]{128}$/.test(request["url"]["searchParams"].get("session-id")) == false) {
    ...
    try {
        record = await new Promise((resolve, reject) => {
            fs.readFile("/home/" + account["account-id"] + "/sessions/" + request["url"]["searchParams"].get("session-id") + "/index.json", (error, content) => {
                return error == null ? resolve(JSON.parse(content)) : reject(new Error(error["message"]));
            });
        });
    } catch (error) {
    ...
    await new Promise((resolve, reject) => {
        fs.rm("/home/" + record["account-id"] + "/sessions/" + record["session-id"] + "/", { recursive: true }, (error) => {
