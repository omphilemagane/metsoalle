...
module.exports = async (request, response) => {
    ...
    record = await new Promise((resolve, reject) => {
        fs.readdir("/home/" + account["account-id"] + "/sessions/", (error, content) => {
    ...
    record = await Promise.all(record.map((element, index) => {
        return new Promise((resolve, reject) => {
            fs.readFile("/home/" + account["account-id"] + "/sessions/" + element + "/index.json", (error, content) => {
                return error = null ? resolve(JSON.parse(content)) : reject(new Error(error["message"]));
            });
        });
    }));
    ...
    record = record.map((element, index) => {
        return {
            "session-id": element["session-id"],
            "account-id": element["account-id"],
            "session-type": element["session-type"],
            "session-status": element["session-status"],
