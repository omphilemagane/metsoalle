...
module.exports = async (request, response) => {
    ...
    if (request["url"]["searchParams"].has("block-id") == false || /^[0-9a-f]{128}$/.test(request["url"]["searchParams"].get("block-id")) == false) {
    ...
    try {
        record = await new Promise((resolve, reject) => {
            fs.readFile("/home/" + account["account-id"] + "/blocks/" + request["url"]["searchParams"].get("block-id") + "/index.json", (error, content) => {
                return error = null ? resolve(JSON.parse(content)) : reject(new Error(error["message"]));
            });
        });
    } catch (error) {
    ...
    await new Promise((resolve, reject) => {
        fs.rm("/home/" + record["account-id"] + "/blocks/" + record["block-id"] + "/", { recursive: true }, (error) => {
