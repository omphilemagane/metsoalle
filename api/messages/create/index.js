...
module.exports = async (request, response) => {
    let account, session, sessiex, theuser, chat, record;
    ...
    if (request["headers"]["content-length"] == undefined || request["headers"]["content-length"] >= 1073742848) {
    ...
    if (request["json"]["data"] == undefined || /^[0-9A-z/+=]{1,1073741824}$/.test(request["json"]["data"]) == false) {
        response.writeHead(400, { "Content-Type": "application/json" });
    ...
    if (request["json"]["mime"] == undefined || /^text\/plain$/.test(request["json"]["mime"]) == false) {
        response.writeHead(400, { "Content-Type": "application/json" });
    ...
    try {
        await new Promise((resolve, reject) => {
            fs.access("/home/" + chat["foreign-id"] + "/chats/" + request["url"]["searchParams"].get("chat-id") + "/index.json", (error) => {
    ...
    record["message-id"] = await new Promise((resolve, reject) => {
        crypto.randomBytes(64, (error, content) => {
            return error == null ? resolve(content.toString("hex")) : reject(new Error(error["message"]));
        });
    });

    record["chat-id"] = chat["chat-id"];

    record["account-id"] = chat["account-id"];

    record["foreign-id"] = chat["foreign-id"];

    record["data"] = request["json"]["data"];

    record["mime"] = request["json"]["mime"];

    record["chat-type"] = "STANDARD";

    record["chat-status"] = "ACTIVE";
    ...
    await new Promise((resolve, reject) => {
        fs.mkdir("/home/" + record["account-id"] + "/chats/" + record["chat-id"] + "/messages/" + record["message-id"] + "/", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });
    ...
    await new Promise((resolve, reject) => {
        fs.mkdir("/home/" + record["foreign-id"] + "/chats/" + record["chat-id"] + "/messages/" + record["message-id"] + "/", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.symlink("/home/" + record["account-id"] + "/chats/" + record["chat-id"] + "/messages/" + record["message-id"] + "/index.json", "/home/" + record["foreign-id"] + "/chats/" + record["chat-id"] + "/messages/" + record["message-id"] + "/index.json", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });
