...
module.exports = async (request, response) => {
    ...
    if (request["json"]["following-status"] == undefined || /^ACTIVE$/.test(request["json"]["following-status"]) == false) {
        response.writeHead(400, { "Content-Type": "application/json" });
    ...
    record["following-status"] = request["json"]["following-status"];
    ...
    await new Promise((resolve, reject) => {
        fs.writeFile("/home/" + record["foreign-id"] + "/followers/" + record["following-id"] + "/index.json", JSON.stringify(record, null, 4), (error) => {
