...
const self = async (request, response, account, session, sessiex) => {
    ...
    if (request["url"]["searchParams"].has("post-id") == false) {
        record = await new Promise((resolve, reject) => {
            fs.readdir("/home/" + account["account-id"] + "/comments/", (error, content) => {
        ...
        record = await Promise.all(record.map((element, index) => {
            return new Promise((resolve, reject) => {
                fs.readFile("/home/" + account["account-id"] + "/comments/" + element + "/index.json", (error, content) => {
        ...
        record = await new Promise((resolve, reject) => {
            fs.readdir("/home/" + account["account-id"] + "/posts/" + request["url"]["searchParams"].get("post-id") + "/comments/", (error, content) => {
                return error == null ? resolve(content) : reject(new Error(error["message"]));
            });
        });
        ...
        record = await Promise.all(record.map((element, index) => {
            return new Promise((resolve, reject) => {
                fs.readFile("/home/" + account["account-id"] + "/posts/" + request["url"]["searchParams"].get("post-id") + "/comments/" + element + "/index.json", (error, content) => {
                    return error == null ? resolve(JSON.parse(content)) : reject(new Error(error["message"]));
                });
            });
        }));
...
const user = async (request, response, account, session, sessiex) => {
    ...
    record = await new Promise((resolve, reject) => {
        fs.readdir("/home/" + theuser["account-id"] + "/posts/" + request["url"]["searchParams"].get("post-id") + "/comments/", (error, content) => {
            return error == null ? resolve(content) : reject(new Error(error["message"]));
        });
    });
    ...
    record = await Promise.all(record.map((element, index) => {
        return new Promise((resolve, reject) => {
            fs.readFile("/home/" + theuser["account-id"] + "/posts/" + request["url"]["searchParams"].get("post-id") + "/comments/" + element + "/index.json", (error, content) => {
                return error == null ? resolve(JSON.parse(content)) : reject(new Error(error["message"]));
            });
        });
    }));
