...
const self = async (request, response, account, session, sessiex) => {
    ...
    record["comment-id"] = await new Promise((resolve, reject) => {
        crypto.randomBytes(64, (error, content) => {
            return error == null ? resolve(content.toString("hex")) : reject(new Error(error["message"]));
        });
    });

    record["post-id"] = request["url"]["searchParams"].get("post-id");

    record["account-id"] = account["account-id"];

    record["foreign-id"] = account["account-id"];

    record["caption"] = request["json"]["caption"];

    record["comment-type"] = "STANDARD";

    record["comment-status"] = "ACTIVE";
    ...
    await new Promise((resolve, reject) => {
        fs.mkdir("/home/" + record["account-id"] + "/comments/" + record["comment-id"] + "/", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.writeFile("/home/" + record["account-id"] + "/comments/" + record["comment-id"] + "/index.json", JSON.stringify(record, null, 4), (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.mkdir("/home/" + record["foreign-id"] + "/posts/" + record["post-id"] + "/comments/" + record["comment-id"] + "/", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.symlink("/home/" + record["account-id"] + "/comments/" + record["comment-id"] + "/index.json", "/home/" + record["foreign-id"] + "/posts/" + record["post-id"] + "/comments/" + record["comment-id"] + "/index.json", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });
...
module.exports = async (request, response) => {
    ...
    if (request["headers"]["content-length"] == undefined || request["headers"]["content-length"] >= 5120) {
    ...
    try {
        request["caption"] = await new Promise((resolve, reject) => {
            const timeout = setTimeout(reject, 15000, 408);

            const caption = [];

            request.on("caption", (chunk) => {
                caption.push(chunk);
            });

            request.on("end", () => {
                if (caption.reduce((index, element) => { return index + element["length"]; }, 0) == request["headers"]["content-length"]) {
                    resolve(Buffer.concat(caption));
                } else {
                    reject(400);
                }
            });
    ...
    try {
        request["json"] = JSON.parse(request["caption"]);
    } catch (error) {
        response.writeHead(400, { "Content-Type": "application/json" });
    ...
    if (request["json"]["caption"] == undefined || /^(?!\s).{1,4096}(?<!\s)$/.test(request["json"]["caption"]) == false) {
        response.writeHead(400, { "Content-Type": "application/json" });
