...
module.exports = async (request, response) => {
    ...
    if (request["json"]["caption"] == undefined || /^(?!\s).{0,4096}(?<!\s)$/.test(request["json"]["caption"]) == false) {
        response.writeHead(400, { "Content-Type": "application/json" });
    ...
    if (request["json"]["post-type"] == undefined || /^(ARCHIVE|DRAFT|STANDARD)$/.test(request["json"]["post-type"]) == false) {
        response.writeHead(400, { "Content-Type": "application/json" });
    ...
    try {
        record = await new Promise((resolve, reject) => {
            fs.readFile("/home/" + account["account-id"] + "/posts/" + request["url"]["searchParams"].get("post-id") + "/index.json", (error, content) => {
                return error == null ? resolve(JSON.parse(content)) : reject(new Error(error["message"]));
            });
        });
    } catch (error) {
    ...
    record["post-type"] = request["json"]["post-type"];
