...
module.exports = async (request, response) => {
    ...
    if (request["headers"]["content-length"] == undefined || request["headers"]["content-length"] >= 1073741824) {
    ...
    if (Array.isArray(request["json"]) == false || request["json"]["length"] == 0) {
        response.writeHead(400, { "Content-Type": "application/json" });
    ...
    for (let i = 0; i < request["json"]["length"]; i++) {
        const isWebm = request["json"][i][0] === 0x1A && request["json"][i][1] === 0x45 && request["json"][i][2] === 0xDF && request["json"][i][3] === 0xA3;

        const isWebp = request["json"][i][0] === 0x52 && request["json"][i][1] === 0x49 && request["json"][i][2] === 0x46 && request["json"][i][3] === 0x46 && request["json"][i][8] === 0x57 && request["json"][i][9] === 0x45 && request["json"][i][10] === 0x42 && request["json"][i][11] === 0x50;
        ...
        if (isWebm == true && /^data\:video\/webm\;base64\,[0-9A-z/+=]{1024,}$/.test(request["json"][i]) == false) {
            response.writeHead(400, { "Content-Type": "application/json" });
        ...
        if (isWebp == true && /^data\:image\/webp\;base64\,[0-9A-z/+=]{1024,}$/.test(request["json"][i]) == false) {
            response.writeHead(400, { "Content-Type": "application/json" });
    ...
    record["post-id"] = await new Promise((resolve, reject) => {
    ...
    record["carousel"] = request["json"];

    record["caption"] = "";

    record["post-type"] = "DRAFT";

    record["post-status"] = "ACTIVE";
    ...
    await new Promise((resolve, reject) => {
        fs.mkdir("/home/" + record["account-id"] + "/posts/" + record["post-id"] + "/", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.writeFile("/home/" + record["account-id"] + "/posts/" + record["post-id"] + "/index.json", JSON.stringify(record, null, 4), (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.mkdir("/home/" + record["account-id"] + "/posts/" + record["post-id"] + "/likes/", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.mkdir("/home/" + record["account-id"] + "/posts/" + record["post-id"] + "/comments/", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.mkdir("/home/" + record["account-id"] + "/posts/" + record["post-id"] + "/bookmarks/", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.mkdir("/home/" + record["account-id"] + "/posts/" + record["post-id"] + "/reports/", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });
