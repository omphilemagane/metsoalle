...
module.exports = async (request, response) => {
    ...
    record["following-id"] = await new Promise((resolve, reject) => {
    ...
    record["following-type"] = "STANDARD";

    record["following-status"] = theuser["account-type"] == "PRIVATE" ? "PENDING" : "ACTIVE";
    ...
    await new Promise((resolve, reject) => {
        fs.mkdir("/home/" + record["account-id"] + "/following/" + record["following-id"] + "/", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.writeFile("/home/" + record["account-id"] + "/following/" + record["following-id"] + "/index.json", JSON.stringify(record, null, 4), (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.mkdir("/home/" + record["foreign-id"] + "/followers/" + record["following-id"] + "/", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.symlink("/home/" + record["account-id"] + "/following/" + record["following-id"] + "/index.json", "/home/" + record["foreign-id"] + "/followers/" + record["following-id"] + "/index.json", (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });
