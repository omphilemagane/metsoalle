...
module.exports = async (request, response) => {
    ...
    try {
        record = await new Promise((resolve, reject) => {
            fs.readFile("/home/" + account["account-id"] + "/reports/" + request["url"]["searchParams"].get("report-id") + "/index.json", (error, content) => {
                return error == null ? resolve(JSON.parse(content)) : reject(new Error(error["message"]));
            });
        });
    } catch (error) {
    ...
    await new Promise((resolve, reject) => {
        fs.rm("/home/" + record["foreign-id"] + "/posts/" + record["post-id"] + "/reports/" + record["report-id"] + "/", { recursive: true }, (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });

    await new Promise((resolve, reject) => {
        fs.rm("/home/" + record["account-id"] + "/reports/" + record["report-id"] + "/", { recursive: true }, (error) => {
            return error == null ? resolve() : reject(new Error(error["message"]));
        });
    });
